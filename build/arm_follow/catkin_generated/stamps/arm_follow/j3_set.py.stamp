...
Utility to rotate Sawyer right_j3 (elbow) to a target angle.
Usage:
    rosrun arm_follow j3_set.py --angle -0.2
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Set Sawyer right_j3 elbow angle.")
...
def main():
    rospy.init_node("j3_set", anonymous=False)
    args = parse_args()

    target = float(np.clip(args.angle, J3_MIN, J3_MAX))

    limb = intera_interface.Limb("right")
    limb.set_joint_position_speed(args.speed)

    current = limb.joint_angles()
    if "right_j3" not in current:
        rospy.logerr("Joint right_j3 not available.")
        sys.exit(1)

    goal = current.copy()
    goal["right_j3"] = target

    rospy.loginfo("Moving right_j3 to %.3f rad (clipped from %.3f)", target, args.angle)
    limb.move_to_joint_positions(goal, timeout=args.timeout, threshold=args.threshold)
    rospy.loginfo("right_j3 set successfully.")
