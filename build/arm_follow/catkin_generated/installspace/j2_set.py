...
Utility to rotate Sawyer right_j2 (shoulder roll) to a target angle.
Usage:
    rosrun arm_follow j2_set.py --angle 0.3
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Set Sawyer right_j2 shoulder roll angle.")
...
def main():
    rospy.init_node("j2_set", anonymous=False)
    args = parse_args()

    target = float(np.clip(args.angle, J2_MIN, J2_MAX))

    limb = intera_interface.Limb("right")
    limb.set_joint_position_speed(args.speed)

    current = limb.joint_angles()
    if "right_j2" not in current:
        rospy.logerr("Joint right_j2 not available.")
        sys.exit(1)

    goal = current.copy()
    goal["right_j2"] = target

    rospy.loginfo("Moving right_j2 to %.3f rad (clipped from %.3f)", target, args.angle)
    limb.move_to_joint_positions(goal, timeout=args.timeout, threshold=args.threshold)
    rospy.loginfo("right_j2 set successfully.")
