  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_core_msgs::IONodeConfiguration_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.devices.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::intera_core_msgs::IOComponentConfiguration_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.devices[i]);
    }
    if (v.devices.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.plugins.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::intera_core_msgs::IOComponentConfiguration_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.plugins[i]);
    }
    if (v.plugins.empty() || false)
      s << "]";
  }
