  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_core_msgs::SolvePositionFKResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.isValid.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.isValid[i]);
    }
    if (v.isValid.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "inCollision: ";
    if (v.inCollision.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.inCollision.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<uint8_t>::stream(s, true ? std::string() : indent + "    ", v.inCollision[i]);
    }
    if (v.inCollision.empty() || true)
      s << "]";
  }
