...
struct Printer< ::intera_core_msgs::JointLimits_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_core_msgs::JointLimits_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "position_lower: ";
    if (v.position_lower.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.position_lower.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.position_lower[i]);
    }
    if (v.position_lower.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "position_upper: ";
    if (v.position_upper.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.position_upper.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.position_upper[i]);
    }
    if (v.position_upper.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.accel.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.accel[i]);
    }
    if (v.accel.empty() || true)
      s << "]";
    if (true || !indent.empty())
