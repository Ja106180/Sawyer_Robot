  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_core_msgs::IODeviceStatus_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.ports.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::intera_core_msgs::IODataStatus_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.ports[i]);
    }
    if (v.ports.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.signals.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::intera_core_msgs::IODataStatus_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.signals[i]);
    }
    if (v.signals.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.commands.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<ros::Time>::stream(s, true ? std::string() : indent + "    ", v.commands[i]);
    }
    if (v.commands.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.responses.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.responses[i]);
    }
    if (v.responses.empty() || true)
      s << "]";
  }
