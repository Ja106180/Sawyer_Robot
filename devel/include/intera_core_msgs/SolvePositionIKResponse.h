  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_core_msgs::SolvePositionIKResponse_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.joints.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::sensor_msgs::JointState_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.joints[i]);
    }
    if (v.joints.empty() || false)
      s << "]";
    ...
    for (size_t i = 0; i < v.result_type.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.result_type[i]);
    }
    if (v.result_type.empty() || true)
      s << "]";
  }
