...
struct Printer< ::intera_motion_msgs::JointTrackingError_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::intera_motion_msgs::JointTrackingError_<ContainerAllocator>& v)
  {
    ...
    if (v.position_error.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.position_error.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.position_error[i]);
    }
    if (v.position_error.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (v.velocity_error.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.velocity_error.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.velocity_error[i]);
    }
    if (v.velocity_error.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (v.position_command.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.position_command.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.position_command[i]);
    }
    if (v.position_command.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "velocity_command: ";
    if (v.velocity_command.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.velocity_command.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.velocity_command[i]);
    }
    if (v.velocity_command.empty() || true)
      s << "]";
  }
