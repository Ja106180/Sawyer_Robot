#ifndef _ROS_yolo_car_ObjectDetections_h
#define _ROS_yolo_car_ObjectDetections_h
...
namespace yolo_car
{
  ...
  class ObjectDetections : public ros::Msg
  {
    public:
      ...
      typedef yolo_car::ObjectDetection _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (yolo_car::ObjectDetection*)realloc(this->objects, objects_lengthT * sizeof(yolo_car::ObjectDetection));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(yolo_car::ObjectDetection));
      }
     return offset;
    }

    virtual const char * getType() override { return "yolo_car/ObjectDetections"; };
