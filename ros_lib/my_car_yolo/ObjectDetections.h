#ifndef _ROS_my_car_yolo_ObjectDetections_h
#define _ROS_my_car_yolo_ObjectDetections_h
...
namespace my_car_yolo
{
  ...
  class ObjectDetections : public ros::Msg
  {
    public:
      typedef std_msgs::Header _header_type;
      _header_type header;
      uint32_t objects_length;
      typedef my_car_yolo::ObjectDetection _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (my_car_yolo::ObjectDetection*)realloc(this->objects, objects_lengthT * sizeof(my_car_yolo::ObjectDetection));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(my_car_yolo::ObjectDetection));
      }
     return offset;
    }

    virtual const char * getType() override { return "my_car_yolo/ObjectDetections"; };
    virtual const char * getMD5() override { return "4612b7e38ebc3d1f73882710c9d084e0"; };
